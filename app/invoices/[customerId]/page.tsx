...
import { getInvoice, getInvoices, createInvoice, createInvoiceFromEstimate, updateInvoice, Invoice, InvoiceItem } from '@/lib/invoices';
...
export default function InvoicePage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [customerData, invoicesData, estimatesData] = await Promise.all([
      ...
      if (invoicesData.length > 0) {
        console.log('Loading invoice detail for ID:', invoicesData[0].id);
        const invoiceData = await getInvoice(invoicesData[0].id);
        console.log('Invoice detail loaded:', invoiceData);
        ...
        console.log('No invoices found for customerId:', customerId);
        setInvoice(null);
      }
    } catch (error) {
  ...
  const handleCopyFromEstimate = async (estimateId: string) => {
    try {
      ...
      setFormData({
        docNo: newInvoice.docNo || '',
        status: newInvoice.status,
        items: newInvoice.items || [],
        cremationProcessType: (newInvoice as any).cremationProcessType || '',
        altarPlaceType: (newInvoice as any).altarPlaceType || '',
        ceilingHeight: (newInvoice as any).ceilingHeight || '',
        estimateStaff: (newInvoice as any).estimateStaff || '',
        ceremonyStaff: (newInvoice as any).ceremonyStaff || '',
        transportStaff: (newInvoice as any).transportStaff || '',
        decorationStaff: (newInvoice as any).decorationStaff || '',
        returnStaff: (newInvoice as any).returnStaff || '',
      });
      alert('見積からコピーしました');
    } catch (error) {
      console.error('Failed to copy from estimate:', error);
      alert('見積からのコピーに失敗しました');
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (invoice) {
        ...
        alert('更新しました');
        // 更新後は請求書編集画面に留まる（データを再読み込み）
        await loadData();
      } else {
        const newInvoice = await createInvoice(customerId, data);
        alert('登録しました');
        // 新規作成後は請求書編集画面に遷移（データを再読み込み）
        await loadData();
      }
    } catch (error) {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <h1 style={{ marginBottom: '2rem' }}>請求書 {invoice ? '編集' : '作成'}</h1>
      ...
      {!invoice && estimates.length > 0 && (
        ...
        >
          <h3 style={{ marginBottom: '1rem' }}>見積からコピー</h3>
