...
export default function InvoicePdfPage() {
    ...
    const loadData = async () => {
        try {
            const [invoiceData, companyData] = await Promise.all([getInvoice(invoiceId), getCompanyProfile()])
            ...
            >
                ...
                {invoice.customer && (
                    <div style={{ marginBottom: '2rem' }}>
                        <p>
                            <strong>故人名:</strong> {invoice.customer.deceasedName}
                        </p>
                        <p>
                            <strong>受付日:</strong>{' '}
                            {invoice.customer.receptionAt
                                ? new Date(invoice.customer.receptionAt).toLocaleDateString('ja-JP')
                                : ''}
                        </p>
                        <p>
                            <strong>喪主名:</strong> {invoice.customer.chiefMournerName}
                        </p>
                        <p>
                            <strong>住所:</strong> {invoice.customer.chiefMournerAddress}
                        </p>
                    </div>
                )}
                ...
                <div style={{ textAlign: 'right', marginBottom: '2rem' }}>
                    ...
                    <p>
                        <strong>合計: ¥{invoice.total.toLocaleString()}</strong>
                    </p>
                    <p>会費入金額: ¥{invoice.membershipPaidAmount.toLocaleString()}</p>
                    <p>
                        <strong>差引合計: ¥{invoice.grandTotal.toLocaleString()}</strong>
                    </p>
                </div>
