...
export default function EstimatePdfPage() {
    ...
    const loadData = async () => {
        try {
            const [estimateData, companyData] = await Promise.all([getEstimate(estimateId), getCompanyProfile()])
            ...
            >
                ...
                {estimate.customer && (
                    <div style={{ marginBottom: '2rem' }}>
                        <p>
                            <strong>故人名:</strong> {estimate.customer.deceasedName}
                        </p>
                        <p>
                            <strong>受付日:</strong>{' '}
                            {estimate.customer.receptionAt
                                ? new Date(estimate.customer.receptionAt).toLocaleDateString('ja-JP')
                                : ''}
                        </p>
                        <p>
                            <strong>喪主名:</strong> {estimate.customer.chiefMournerName}
                        </p>
                        <p>
                            <strong>住所:</strong> {estimate.customer.chiefMournerAddress}
                        </p>
                    </div>
                )}
                ...
                >
                    <thead>
                        <tr style={{ backgroundColor: '#f5f5f5' }}>
                            ...
                            <th style={{ padding: '0.75rem', border: '1px solid #ddd', textAlign: 'right' }}>数量</th>
                            <th style={{ padding: '0.75rem', border: '1px solid #ddd', textAlign: 'right' }}>単価</th>
                ...
                <div style={{ textAlign: 'right', marginBottom: '2rem' }}>
                    ...
                    <p>
                        <strong>合計: ¥{estimate.total.toLocaleString()}</strong>
                    </p>
                    <p>会費入金額: ¥{estimate.membershipPaidAmount.toLocaleString()}</p>
                    <p>
                        <strong>差引合計: ¥{estimate.grandTotal.toLocaleString()}</strong>
                    </p>
                </div>
                ...
                {company && (
                    ...
                    >
                        ...
                        {company.bank1Name && (
                            <p>
                                {company.bank1Name} {company.bank1Branch} {company.bank1Type} {company.bank1Account}{' '}
                                {company.bank1Holder}
                            </p>
                        )}
                        {company.bank2Name && (
                            <p>
                                {company.bank2Name} {company.bank2Branch} {company.bank2Type} {company.bank2Account}{' '}
                                {company.bank2Holder}
                            </p>
                        )}
                    </div>
